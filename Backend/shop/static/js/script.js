...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (accessToken) {
        loginLogoutButton.innerHTML = '<img src="http://127.0.0.1:8000/static/img/log-out.svg" alt="Logout">';
        loginLogoutButton.addEventListener("click", function () {
            localStorage.removeItem("access_token");
            window.location.href = "http://127.0.0.1:5501/login.html";
        });
    } else {
        loginLogoutButton.innerHTML = '<img src="http://127.0.0.1:8000/static/img/log-in.svg" alt="Logout">';
...
// Устанавливаем изначальное значение рейтинга и комментария 
let initialRating = 3; // Измените на нужное изначальное значение
let initialComment = "Мені сподобалося"; // Измените на нужный изначальный комментарий

ratingInputs[initialRating - 1].checked = true; // Выбираем соответствующий input для изначального рейтинга
commentTextarea.value = initialComment; // Устанавливаем изначальный комментарий

btn.onclick = () => {
    widget.style.display = "none";
    post.style.display = "block";

    // Обработчик для кнопки "Редагувати"
    editBtn.onclick = () => {
